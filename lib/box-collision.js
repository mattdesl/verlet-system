module.exports = function(vec) {
    ...
    var EDGE = 1 //TODO: fix edge bug where vertices are "locked" to a side
    ...
    return function collider(p, velocity, min, max, friction) {
        ...
        //bounce and clamp
        for (i=0; i<n; i++)
            if (typeof min[i] === 'number' && p.position[i]-radius < min[i]-EDGE) {
                reflect[i] = velocity[i] > 0 ? -1 : 1
                p.position[i] = min[i]+radius
                hit = true
            }
        for (i=0; i<n; i++)
            if (typeof max[i] === 'number' && p.position[i]+radius > max[i]-EDGE) {
                reflect[i] = velocity[i] > 0 ? -1 : 1
